...
위키피디아는 [Docker](https://www.docker.com/) 를 다음과 같이 정의합니다.

> 도커는 리눅스에서 운영 체제 수준 가상화의 추상화 및 자동화 계층을 추가로 제공하여 응용 프로그램들을 소프트웨어 컨테이너 안에 배치시키는 일을 자동화하는 오픈 소스 프로젝트이다.

와! 정말 깁니다. 더 간단히 말하면, 도커는 개발자나 시스템 관리자등이 리눅스와 같은 호스트 운영체제 위에서 어플리케이션을 (_컨테이너_ 라고 불리는) 샌드박스에 쉽게 배포하도록 도와주는 툴입니다. 도커의 가장 큰 장점은 유저가 소프트웨어 개발에 필요한 **어플리케이션과 모든 의존성들을 표준화된 규격으로 패키징**하는 것입니다. 컨테이너는 가상머신과 달리 오버헤드가 작아 시스템과 리소스를 더 효율적으로 사용할 수 있습니다.

### 컨테이너란 무엇인가요?

오늘날 응용 프로그램을 실행하기 위한 업계 표준은 가상 머신(VMs)을 사용하는 것입니다. 가상머신은 게스트 OS 안에서 어플리케이션을 실행합니다. 그리고 그 게스트 OS는 서버의 호스트 OS에 의해 구동되는 가상 하드웨어에서 실행됩니다.

가상머신은 어플리케이션을 완전히 프로세스 격리하는데 탁월합니다. 호스트 운영체제와 게스트 OS 시스템속 소프트웨어는 서로 거의 영향을 끼치지 않습니다. 하지만 이 격리에는 비싼 대가가 있습니다. 게스트 OS를 위한 하드웨어 가상화에 어마어마한 컴퓨팅 오버헤드가 필요합니다.

컨테이는 접근 방식이 다릅니다. 호스트 운영체제의 저레벨 영역을 활용하여 컨테이너는 작은 컴퓨팅 파워로도 대부분의 가상머신 격리를 지원합니다.

### 왜 컨테이너를 사용하나요?

컨테이너는 어플리케이션을 실제로 구동되는 환경으로부터 추상화시키기 위해 논리적 패키징 메커니즘을 제공합니다. 이 디커플링이 컨테이너 기반 어플리케이션을 프라이빗 데이터 센터든, 퍼블릭 클라우드든, 심지어 개발자의 노트북이든지 쉽고 지속적으로 배포할 수 있게 도와주죠. 이것은 다른 어플리케이션들과 분리되고 어디서든 실행할 수 있는 예측 가능한 환경을 구성할 수 있게 합니다.

이식성을 떠나 운영 관점에서 보면, 컨테이너는 리소스에 대해 더 세분화된 제어를 제공합니다. 이는 당신의 컴퓨터 자원을 더 잘 활용할 수 있도록 인프라를 효율적으로 만들어줍니다.
...
이러한 장점으로 인해, 컨테이너(& 도커)는 널리 채택되고 있습니다. Google, Facebook, Netflix 그리고 Salesforce 같은 회사들은 거대한 엔지니어링 팀의 생산성을 높이고 컴퓨터 자원을 효율적으로 하기 위해 컨테이너를 활용합니다. 사실 Google은 컨테이너가 모든 데이터센터의 필요 자체를 없애버릴 것이라 믿고 있습니다.

### 이 튜토리얼을 통해 어떤걸 배우게 되나요?

이 튜토리얼은 당신이 원스톱으로 도커를 만져보는 것을 목표로 합니다. 도커에 대해 완전히 이해하진 못하더라도 직접 자신의 웹앱을 만들어 클라우드에 배포하는 경험을 드립니다. 우리는 정적 사이트를 [Amazon Web Services](http://aws.amazon.com)에 배포할 예정입니다. 또한 [Elastic Beanstalk](https://aws.amazon.com/elasticbeanstalk/) 와 [Elastic Container Service](https://aws.amazon.com/ecs/)를 사용해 [EC2](https://aws.amazon.com/ec2/)에 두 동적 웹앱을 배포할 겁니다. 만약 배포 경험이 없더라도 이 튜토리얼만 있으면 문제 없습니다.
